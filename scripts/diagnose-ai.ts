...
const SUPABASE_ANON_KEY = import.meta.env?.VITE_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNxc3FlZHZoaG55aHd4YWt1anlmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAxMjM5NjEsImV4cCI6MjA4NTY5OTk2MX0.4xJbf6fTBqsd4xagMcUuibW7XAeT-vf5UZWXAXvyhds";
...
async function diagnose() {
  ...
  try {
    ...
    results.step1.details = (error as Error).message;
  ...
  try {
    ...
    if (response.ok || response.status === 405) {
      // 405 Method Not Allowed 表示函数存在但不支持 OPTIONS
    ...
    results.step2.details = (error as Error).message;
    console.error(`   ❌ 请求错误: ${(error as Error).message}`);
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      // 分析具体错误
      if (data.error?.includes('DeepSeek API 未配置')) {
    ...
    results.step3.details = (error as Error).message;
    console.error(`   ❌ 调用错误: ${(error as Error).message}`);
  }
